...
export function encode(message: Message): Buffer {
  ...
  if (type < 0 || type > 15) {
    throw new TypeError(
      "Packet type must have a value between 0 and 15 (inclusive)"
    );
  }
  ...
  if (payloadLength < 0 || payloadLength > 1023) {
    throw new TypeError(
      "Payload lengths must be between 0 and 1023 (inclusive)."
    );
  }
  ...
  if (ackNum < 0 || ackNum > ACK_NUM.MAX) {
    throw new TypeError(
      `AckNums must be between 0 and ${ACK_NUM.MAX} (inclusive).`
    );
  }
  ...
  if (offset !== null) {
    ...
    if (offset < 0 || offset > 65535) {
      throw new TypeError(
        "Offsets must be between 0 and 65535 (inclusive) or null."
      );
    }
  ...
  debug(
    () =>
      `Encoded message is ${packetBuffer.toString(
        "hex"
